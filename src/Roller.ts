export class Roller {
    ...
    constructor(faces: number) {
        this._faces = faces;
        //initialize distribution and last
    ...
    roll(value: number): number {
        if(value > 0 && value <= this._faces) {
            this._last = value;
            var previousValue = this._distribution.get(value);
            this._distribution.set(value, previousValue+1);
            return value;
        }
